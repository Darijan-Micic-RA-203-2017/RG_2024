...
// It is possible to load a character glyph, retrieve its metrics and generate a texture each time a character needs to
// be rendered to the screen, but it would be inefficient to do this each frame. It is smarter to store the generated
// data somewhere in the application and query it whenever a character needs to be rendered. This convenient structure
// serves that purpose. It is then stored in a map.
struct Character {
...
unsigned int generateTextureForLoadedGlyph(FT_Face &face)
{
	...
	// For this reason, it is needed to store each byte of the bitmap buffer as the texture's single color value.
	// This is accomplished by creating a texture where each byte corresponds to the texture color's red component
...
void extractNecessaryInfoFromFaceOn(std::string pathOfFace)
{
	...
	// A TrueType font is a collection of character glyphs not defined by pixels or any other non-scalable solution, but
	// by mathematical equations (combinations of splines). Similar to vector images, the rasterized font images can be
	// procedurally generated based on the preferred font height the user would like to obtain them in. Using TrueType
	// fonts enables easy rendering of character glyphs of various sizes without any loss of quality. 
	FT_Face face;
	// if (FT_New_Face(ft, "Resources/Fonts/times.ttf", 0L, &face))
	...
	// Disable the byte-alignment restriction. If a single byte is used to represent each of the colors of a texture,
	// one restriction of OpenGL needs to be taken care of.
	...
	// per pixel. However, if only a single byte is used per pixel, the texture can have any possible width.
	// Its unpack alignment must be set to 1 to ensure there are no alignment issues, which could cause segmentation faults.
	...
	for (unsigned char c = 0U; c < 128U; c++)
	{
		...
		// A FreeType face hosts a collection of glyphs. One of those glyphs can be set as the active glyph by calling
		// the "FT_Load_Char" method. By setting "FT_LOAD_RENDER" as one of the loading flags, FreeType library is told
		// to create an 8-bit grayscale bitmap image. That image can be accessed in this way: face->glyph->bitmap.
...
void renderText(ShaderProgram &shaderProgram, std::string text, float x, float y, float scale, glm::vec3 color)
{
	...
	for (c = text.begin(); c != text.end(); c++)
	{
		...
		// necessary for calculating this distance (offset). The length of this vector can be calculated by subtracting
		// "bearingY" from the glyph's (bitmap's) height. This value is then 0.0F for characters that rest on the
		// baseline (like 'X') and positive for characters that reside slightly below the baseline (like 'g' or 'j').
		float yPos = y - (ch.size.y - ch.bearing.y) * scale;

		float width = ch.size.x * scale;
		float height = ch.size.y * scale;
		// Update the VBO for each character. Multidimensional vertices array has 6 rows because it takes 6 vertices to
		// draw a rectangle and 4 columns because it takes.
		float vertices[6][4] = {
